            ...
            <tbody>
                @foreach ($countries as $index => $country)
                    <tr class="border-b">
                        ...
                        <td class="p-2">
                            <a href="{{ route('countries.edit', $country->id) }}" class="btn btn-primary" style="width: 50px; display: inline-block;">Edit</a>
                            <form action="{{ route('categories.destroy', ['country' => $country->id]) }}" method="post"class="inline" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
