            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($cities as $index => $city)
                    <tr class="border-b">
                        ...
                        <td class="p-2">{{ optional($city->country)->name }}</td> <!-- Accessing country name with optional chaining -->
                        <td class="p-2">{{ $city->city_name }}</td>
                        {{-- <td class="p-2">{{ $cityNames->implode(', ') }}</td> --}}
                        <td class="p-2">
                            <a href="{{ route('cities.edit', $city->id) }}" class="btn btn-primary" style="width: 50px; display: inline-block;">Edit</a>
                            <form action="{{ route('cities.destroy', $city->id) }}" method="post" class="inline" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
